...
export function PayoutsFilters() {
  ...
  const validationSchema = z.object({
    payment_id: z.string().regex(/^\d{9}$|^\d{10}$/, 'Payment ID должен иметь 9 или 10 цифр'),
  ...
  const inputsData = [
    {label: 'Payout ID', storeKey: 'id'},
    {label: 'Order ID', storeKey: 'order_id'},
    {label: 'Номер счёта', storeKey: 'accountNumber'}
  ]
  ...
  console.log('PAYOUT FILTERS = ', filters)
                  ...
                  selectsData.map((select) => (
                    ...
                    >
                      <Controller
                        ...
                        defaultValue={filters.payouts?.[select.storeKey] ?? null}
                        render={({ field }) => (
                          <FormControl error={Boolean(errors[select.storeKey])} fullWidth>
                            ...
                            <Select
                              ...
                              onChange={(event) => {
                                ...
                                filtersHandler("payouts", select.storeKey, value);
                              }}
                            >
                      ...
                      inputsData.map((input) => (
                        ...
                        >
                          <Controller
                            control={control}
                            name={input.storeKey}
                            defaultValue={filters.payouts?.[input.storeKey] ?? null}
                            render={({ field }) => (
                              <FormControl error={Boolean(errors[input.storeKey])} fullWidth>
                                ...
                                <OutlinedInput
                                  ...
                                  onChange={(event) => {
                                    const value = event.target.value;
                                    field.onChange(value);
                                    filtersHandler("payouts", input.storeKey, value);
                                  }}
